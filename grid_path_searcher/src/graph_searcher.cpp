...
{   
    gl_xl = global_xyz_l(0);//-25
    gl_yl = global_xyz_l(1);//-25
    gl_zl = global_xyz_l(2);//0

    gl_xu = global_xyz_u(0);//25
    gl_yu = global_xyz_u(1);//25
    gl_zu = global_xyz_u(2);//5.0

    GLX_SIZE = max_x_id;//250 地图x轴元素个数
    GLY_SIZE = max_y_id;//250 地图Y轴元素个数
    GLZ_SIZE = max_z_id;//25 地图z轴元素个数
    ...
    GridNodeMap = new GridNodePtr ** [GLX_SIZE];//三维指针
    for(int i = 0; i < GLX_SIZE; i++){
        GridNodeMap[i] = new GridNodePtr * [GLY_SIZE];//二维指针
        for(int j = 0; j < GLY_SIZE; j++){
            GridNodeMap[i][j] = new GridNodePtr [GLZ_SIZE];
            for( int k = 0; k < GLZ_SIZE;k++){
                Vector3i tmpIdx(i,j,k);//该格子的索引下标
                Vector3d pos = GridIndex2coord(tmpIdx);//该格子的全局坐标
